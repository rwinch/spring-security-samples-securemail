...
create table message (
  ...
  fromUser bigint,
  toUser bigint,
  FOREIGN KEY(fromUser) REFERENCES message_user(id),
  FOREIGN KEY(toUser) REFERENCES message_user(id)
);

insert into message_user(id,email,password,firstName,lastName) values (1,'rob@example.org','penguin','Rob','Winch');
insert into message_user(id,email,password,firstName,lastName) values (2,'luke@example.com','lion','Luke','Taylor');
